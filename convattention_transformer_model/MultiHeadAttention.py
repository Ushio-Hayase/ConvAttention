...
class MultiHeadAttentionLayer(nn.Module):
    def __init__(self, d_model: int, num_heads: int):
        ...
        self.w_q: nn.Linear = nn.Conv1d(d_model, d_model, 3, 1, 1)
        self.w_k: nn.Linear = nn.Conv1d(d_model, d_model, 3, 1, 1)
        self.w_v: nn.Linear = nn.Conv1d(d_model, d_model, 3, 1, 1)
    ...
    def concat(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        return inputs.transpose(1, 2).contiguous().view(batch_size, seq_len, self.d_model)
